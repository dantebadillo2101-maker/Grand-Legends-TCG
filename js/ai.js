...
function scoreAICard(card, config) {
  ...
  if (card.id && (card.id.startsWith('E') || card.id.startsWith('R'))) score += 350;
  if (card.id === 'E35' || card.id === 'E36' || card.id === 'E25') score += 1800;
  return score;
}
...
function chooseAIEvolutionCard(cards, config) {
  ...
  return valid.slice().sort((a, b) => scoreAICard(b, config) - scoreAICard(a, config))[0];
}
